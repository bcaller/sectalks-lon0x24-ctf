...
class CaptureTheFlagOracle(PaddingOracle):
    def __init__(self, iv_hex: bytes, sock):
        self.iv = iv_hex + b'\n'
        self.socket = sock
        super().__init__(8)
    ...
    def _oracle_says_padding_correct(self, ciphertext: bytes) -> bool:
        line = b''
        while b'IV: ' not in line:
            line += self.socket.recv(512)
        self.socket.sendall(self.iv)
        line = b''
        while b'MESSAGE: ' not in line:
            line += self.socket.recv(512)
        self.socket.sendall(ciphertext.hex().encode() + b'\n')
        line = b''
        while b'SUCCESS' not in line and b'ERROR' not in line:
            line += self.socket.recv(512)
        self.socket.sendall(b'\n')
        return b'SUCCESS' in line


def run(iv_hex, message_hex, ip='127.0.0.1', port=9000):
    import socket
    logging.basicConfig(
        level=logging.INFO,
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        oracle = CaptureTheFlagOracle(
            iv_hex,
            sock,
        )
        assert oracle._oracle_says_padding_correct(bytes.fromhex(message_hex))
        result = attack(bytes.fromhex(message_hex), oracle, 8)
        print(result)
